...
@torch.no_grad()
def evaluate_whole_graph(args, model, datasets_tr, datasets_val, datasets_te, eval_func, data_loaders=None):
    model.eval()
    accs, test_outs = [], []
    y_tr, out_tr, y_val, out_val = [], [], [], []
    for i, dataset in enumerate(datasets_tr):
        out = model.inference(dataset)
        if args.dataset == 'elliptic':
            y_tr.append(dataset.label[dataset.mask])
            out_tr.append(out[dataset.mask])
        else:
            y_tr.append(dataset.label)
            out_tr.append(out)
    acc_tr = eval_func(torch.cat(y_tr, dim=0), torch.cat(out_tr, dim=0))
    for i, dataset in enumerate(datasets_val):
        out = model.inference(dataset)
        if args.dataset == 'elliptic':
            y_val.append(dataset.label[dataset.mask])
            out_val.append(out[dataset.mask])
        else:
            y_val.append(dataset.label)
            out_val.append(out)
    ...
    for i, dataset in enumerate(datasets_te):
        ...
        if args.dataset == 'elliptic':
            ...
            out_te.append(out[dataset.mask])
        else:
            y_te.append(dataset.label)
            out_te.append(out)
        if i % 4 == 0 or i == len(datasets_te) - 1:
