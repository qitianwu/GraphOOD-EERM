...
def parse_method_base(args, datasets, device):
    n, c, d = datasets[0].n, datasets[0].c, datasets[0].d
    if args.gnn == 'gcn':
...
def parse_method_ours(args, datasets, device):
    ns = [dataset.n for dataset in datasets]
    c, d = datasets[0].c, datasets[0].d
    if args.gnn == 'gcn':
        model = Model(args, ns, c, d, 'gcn', device).to(device)
    elif args.gnn == 'sage':
        model = Model(args, ns, c, d, 'sage', device).to(device)
    elif args.gnn == 'gat':
        model = Model(args, ns, c, d, 'gat', device).to(device)
    elif args.gnn == 'gpr':
        model = Model(args, ns, c, d, 'gpr', device).to(device)
    else:
        raise ValueError('Invalid method')
    return model
