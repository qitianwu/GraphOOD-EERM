...
@torch.no_grad()
def evaluate_whole_graph(args, model, dataset_tr, dataset_val, datasets_te, eval_func, data_loaders=None):
    model.eval()
    accs, test_outs = [], []
    train_out = model.inference(dataset_tr)
    train_acc = eval_func(dataset_tr.label, train_out)
    valid_out = model.inference(dataset_val)
    valid_acc = eval_func(dataset_val.label[dataset_val.test_mask], valid_out[dataset_val.test_mask])
    accs += [train_acc] + [valid_acc]
    for i, dataset in enumerate(datasets_te):
        out = model.inference(dataset)
        test_outs.append(out[dataset.test_mask])
        accs.append(eval_func(dataset.label[dataset.test_mask], out[dataset.test_mask]))
