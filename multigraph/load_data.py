...
def load_reddit(num):
    assert num in (1, 2, 3, 4, 5), 'Invalid dataset'
    filepath = DATAPATH + f"reddit/graph{num}"
    file_name = filepath + f"/graph{num}.edgelist"
    ...
    features = pkl.load(open(filepath + f"/graph{num}.x.pkl", 'rb'))
    features = np.array(features).astype(np.float32)
    label = pkl.load(open(filepath + f"/graph{num}.y.pkl", 'rb'))
